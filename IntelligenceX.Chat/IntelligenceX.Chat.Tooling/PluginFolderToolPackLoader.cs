...
internal static class PluginFolderToolPackLoader {
    ...
    private static void TryLoadPluginDirectory(
        ...
        var entryAssemblyPath = ResolveEntryAssemblyPath(pluginDirectory, manifest, pluginId, onWarning);
        if (entryAssemblyPath is null) {
            return;
        }

        PreloadPluginDependencies(pluginDirectory, entryAssemblyPath, pluginId, onWarning);

        var entryAssembly = TryLoadAssembly(entryAssemblyPath, pluginId, onWarning);
        if (entryAssembly is null) {
            return;
        }

        var candidateTypes = ResolveCandidatePackTypes(entryAssembly, manifest, pluginId, onWarning);
    ...
    private static string? ResolveEntryAssemblyPath(
        ...
        return NormalizePath(dlls[0]);
    }

    private static void PreloadPluginDependencies(string pluginDirectory, string entryAssemblyPath, string pluginId, Action<string>? onWarning) {
        string[] dlls;
        try {
            dlls = Directory
                .EnumerateFiles(pluginDirectory, "*.dll", SearchOption.TopDirectoryOnly)
        ...
        foreach (var dll in dlls) {
            ...
            if (string.Equals(fullPath, entryAssemblyPath, StringComparison.OrdinalIgnoreCase)) {
                continue;
            }
