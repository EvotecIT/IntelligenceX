...
public sealed partial class ChatServiceRoutingTrimTests {
    ...
    [Fact]
    public async Task RunChatOnCurrentThreadAsync_ReusesPriorOutput_WhenRetryReplayMixedCallsAreReordered() {
        using var server = new DeterministicCompatibleHttpServer(
            dropChatCompletionResponseOnRequestIndices: new[] { 2 },
            emitReplayMixedToolCallsAfterDropReordered: true);
        ...
        var request = new ChatRequest {
            RequestId = "req-e2e-replay-mixed-reordered",
            ThreadId = thread.Id,
            Text = "Run the diagnostics workflow to completion.",
            Options = new ChatRequestOptions {
                WeightedToolRouting = false,
