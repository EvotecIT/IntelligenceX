...
internal static class AnalyzeRunCommand {
    ...
    private const string LegacyMaxFileLinesRuleId = "IXLOC001";
    private const int DefaultMaxFileLinesLimit = 700;
    private const string MaxLinesTagPrefix = "max-lines:";
    private const int GeneratedHeaderLinesToInspect = 20;
    ...
    private static IReadOnlyList<AnalysisFindingItem> RunInternalMaintainabilityChecks(string workspace,
        ...
        var sourceFiles = EnumerateCSharpFiles(workspace, warnings)
            ...
            .Where(file => !IsExcludedSourceFile(file.FullPath, file.RelativePath, generatedSuffixes));
    ...
    private static AnalysisPolicyRule? ResolveMaxLinesRule(IReadOnlyList<AnalysisPolicyRule> rules, out int maxLinesLimit) {
        ...
        var legacyRule = rules.FirstOrDefault(rule => IsRuleMatch(rule.Rule, LegacyMaxFileLinesRuleId));
        if (legacyRule is not null) {
            return legacyRule;
        }

        return rules.FirstOrDefault(rule => string.Equals(rule.Rule.Tool, InternalToolName, StringComparison.OrdinalIgnoreCase));
    }
    ...
    private static IReadOnlyCollection<string> ResolveGeneratedSuffixes(AnalysisRule rule) {
        var suffixes = new HashSet<string>(GeneratedSuffixes, StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var tag in rule.Tags) {
            ...
            var value = tag.Substring(GeneratedSuffixTagPrefix.Length).Trim();
    ...
    private static bool IsExcludedSourceFile(string fullPath, string relativePath, IReadOnlyCollection<string> generatedSuffixes) {
        ...
        if ((generatedSuffixes ?? Array.Empty<string>()).Any(suffix => normalized.EndsWith(suffix, StringComparison.OrdinalIgnoreCase))) {
            return true;
        }

        return HasGeneratedFileHeader(fullPath);
    }
    ...
    private static bool HasGeneratedFileHeader(string fullPath) {
        try {
            ...
            for (var i = 0; i < GeneratedHeaderLinesToInspect; i++) {
                ...
                if (GeneratedHeaderMarkers.Any(marker => normalized.Contains(marker, StringComparison.Ordinal))) {
                    return true;
                }
            }
        } catch {
