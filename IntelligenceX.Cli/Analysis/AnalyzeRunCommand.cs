    ...
    private static bool TryAddPackValues(ICollection<string> packs, string? raw, out string? error) {
        ...
        foreach (var value in values) {
            ...
            if (!PackIdRegex.IsMatch(value)) {
                error = $"Invalid pack id '{value}'. Use comma-separated ids like all-50, all-multilang-50, all-security-50, all-security-default, powershell-50, javascript-50, python-50.";
                return false;
            }
