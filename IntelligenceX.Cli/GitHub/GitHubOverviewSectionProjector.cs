...
internal static class GitHubOverviewSectionProjector {
    private const int TrailingDays = 365;
    private static readonly string[] LegendColors = { "#e8e8e8", "#d6ecd3", "#9be9a8", "#40c463", "#216e39" };

    public static UsageTelemetryOverviewProviderSection Project(GitHubOverviewDataSnapshot snapshot) {
        ArgumentNullException.ThrowIfNull(snapshot);
        return snapshot.OwnerImpactOnly
            ? ProjectOwnerImpactOnly(snapshot)
            : ProjectContributionWindow(snapshot);
    }

    private static UsageTelemetryOverviewProviderSection ProjectContributionWindow(GitHubOverviewDataSnapshot snapshot) {
        var calendar = snapshot.Calendar
            ?? throw new InvalidOperationException("GitHub contribution activity is required for the full overview section.");
        var previousYearCalendar = snapshot.PreviousYearCalendar
            ?? throw new InvalidOperationException("GitHub comparison activity is required for the full overview section.");
        var repositoryImpact = snapshot.RepositoryImpact;
        var endUtc = snapshot.EndUtc.Date;
        var startUtc = snapshot.StartUtc.Date;
        ...
        var previousYearEndUtc = snapshot.PreviousYearCalendar is null
            ? CreateComparableYearEnd(previousYearStartUtc.Year, endUtc.Month, endUtc.Day)
            : snapshot.PreviousYearCalendar.Days.LastOrDefault()?.Date switch {
                { } previousYearEndDate => new DateTimeOffset(previousYearEndDate, TimeSpan.Zero),
                _ => CreateComparableYearEnd(previousYearStartUtc.Year, endUtc.Month, endUtc.Day)
            };
        ...
        var additionalInsights = BuildAdditionalInsights(
            snapshot.RequestedLogin?.Trim() ?? calendar.Login,
            endUtc.Date,
            currentYearDays,
            previousYearDays,
            repositoryImpact);
    ...
    private static UsageTelemetryOverviewProviderSection ProjectOwnerImpactOnly(GitHubOverviewDataSnapshot snapshot) {
        var repositoryImpact = snapshot.RepositoryImpact
            ?? throw new InvalidOperationException("GitHub repository impact is required for the owner-only overview section.");
