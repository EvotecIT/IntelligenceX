...
""");
    ...
    private static void TestCopilotRequiresAnExplicitModel() {
        var settings = new ReviewSettings {
            Provider = ReviewProvider.Copilot
        };

        AssertEqual(null, ReviewRunner.ResolveCopilotModel(settings), "copilot default model");
        settings.Model = OpenAIModelCatalog.DefaultModel;
        AssertEqual(OpenAIModelCatalog.DefaultModel, ReviewRunner.ResolveCopilotModel(settings), "explicit model is valid even when it matches the ChatGPT default");
    }
