...
internal static partial class Program {
    ...
    private static void TestPowerShellExampleScriptsUseExportedCmdlets() {
        var workspace = ResolveWorkspaceRoot();
        var examplesDirectory = Path.Combine(workspace, "Module", "Examples");
    ...
    private static void TestPowerShellGeneratedHelpMatchesExports() {
        var workspace = ResolveWorkspaceRoot();
        var exported = LoadExportedCmdletSet(workspace);
        var document = XDocument.Load(Path.Combine(workspace, "Module", "en-US", "IntelligenceX-help.xml"));
        var names = document.Root!.Elements(CommandNs + "command")
            .Select(command => command.Element(CommandNs + "details")?.Element(CommandNs + "name")?.Value)
            .ToArray();
        AssertEqual(true, names.All(name => !string.IsNullOrWhiteSpace(name)), "generated help command names");
        AssertEqual(names.Length, names.Distinct(StringComparer.OrdinalIgnoreCase).Count(), "generated help has no duplicate commands");
        AssertEqual(true, exported.SetEquals(names!), "generated help matches exported commands");
    }
