...
internal static partial class Program {
    ...
    private static void TestSetupProviderCatalogReturnsRecommendedOpenAiModels() {
        ...
        AssertEqual("gpt-6-sol", models[0], "setup provider catalog openai default first");
        AssertEqual(true, models.Contains("gpt-5.5/fast", StringComparer.OrdinalIgnoreCase),
            "setup provider catalog openai fast present");
        AssertEqual(true, models.Contains("gpt-5.5", StringComparer.OrdinalIgnoreCase),
            "setup provider catalog previous default present");
    ...
    private static void TestSetupProviderCatalogDescribesRecommendedModelProfiles() {
        var openAiProfile = IntelligenceX.Cli.Setup.SetupProviderCatalog.TryGetRecommendedModelProfile("openai", "gpt-6-sol");
        AssertEqual(true, openAiProfile.HasValue, "setup provider catalog openai profile exists");
        AssertContainsText(openAiProfile?.Summary ?? string.Empty, "Balanced default", "setup provider catalog openai profile summary");
