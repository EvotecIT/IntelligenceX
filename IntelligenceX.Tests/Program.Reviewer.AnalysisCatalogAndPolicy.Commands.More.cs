...
internal static partial class Program {
    ...
    private static void TestAnalyzeListRulesAllMultilangTierCounts() {
        var workspace = ResolveWorkspaceRoot();
        var (exitAll50, outputAll50) = RunAnalyzeAndCaptureOutput(new[] {
            "list-rules",
            "--workspace",
            workspace,
            "--pack",
        ...
        AssertEqual(0, exitAll50, "analyze list-rules all-50 baseline exit");
        AssertEqual(0, exit50, "analyze list-rules all-multilang-50 exit");
        AssertEqual(0, exit100, "analyze list-rules all-multilang-100 exit");
        AssertEqual(0, exit500, "analyze list-rules all-multilang-500 exit");

        var baselineAll50 = ParseListedRuleCount(outputAll50, "all-50");
        var count50 = ParseListedRuleCount(output50, "all-multilang-50");
