...
description: Guided CLI wizard for setting up IntelligenceX across repositories
collection: docs
layout: docs
nav.weight: 20
---
...
1. **GitHub App** (installation token) - Recommended for org-wide onboarding. You can create an App via the manifest flow. The wizard can save the App profile for reuse.
2. **OAuth device flow** - Fastest for a single repo.
3. **Personal access token** - Use only if required by your org policy.
...
The wizard prints the secret value and instructions for manual paste.

## Explicit Secrets Block

If you want to avoid `secrets: inherit` in the workflow:
...
- If no installations are found for a GitHub App, install it first at `https://github.com/apps/<app-id>/installations`
