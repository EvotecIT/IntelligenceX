...
/// <summary>
/// Returns WinRM policy posture for Domain Controllers OU in one domain (read-only).
/// </summary>
public sealed class AdWinRmPolicyTool : ActiveDirectoryToolBase, ITool {
    private const int MaxViewTop = 5000;

    private static readonly ToolDefinition DefinitionValue = new(
        "ad_winrm_policy",
        "Assess WinRM service policy posture (AllowUnencryptedTraffic, Basic auth) for Domain Controllers OU (read-only).",
        ToolSchema.Object(
    ...
    private sealed record AdWinRmPolicyResult(
        string DomainName,
        ...
        bool PotentiallyInsecure,
        IReadOnlyList<PolicyAttribution> Attribution);

    /// <summary>
    /// Initializes a new instance of the <see cref="AdWinRmPolicyTool"/> class.
    /// </summary>
    public AdWinRmPolicyTool(ActiveDirectoryToolOptions options) : base(options) { }
