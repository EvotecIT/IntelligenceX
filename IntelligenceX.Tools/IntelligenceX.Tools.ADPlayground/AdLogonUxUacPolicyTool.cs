...
/// <summary>
/// Returns Logon UX and UAC policy posture for Domain Controllers OU in one domain (read-only).
/// </summary>
public sealed class AdLogonUxUacPolicyTool : ActiveDirectoryToolBase, ITool {
    private const int MaxViewTop = 5000;

    private static readonly ToolDefinition DefinitionValue = new(
        "ad_logon_ux_uac_policy",
        "Assess Logon UX and UAC policy posture (CTRL+ALT+DEL, last user display, secure desktop) for Domain Controllers OU (read-only).",
        ToolSchema.Object(
    ...
    private sealed record AdLogonUxUacPolicyResult(
        string DomainName,
        ...
        bool? HideLastUserName,
        bool? SecureDesktop,
        IReadOnlyList<PolicyAttribution> Attribution);

    /// <summary>
    /// Initializes a new instance of the <see cref="AdLogonUxUacPolicyTool"/> class.
    /// </summary>
    public AdLogonUxUacPolicyTool(ActiveDirectoryToolOptions options) : base(options) { }
