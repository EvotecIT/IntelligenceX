...
/// <summary>
/// Builds a summary report for Windows failed logon events from an EVTX file (read-only, restricted to allowed roots).
/// </summary>
public sealed class EventLogEvtxFailedLogonsReportTool : EventLogToolBase, ITool {
    private static readonly ToolDefinition DefinitionValue = new(
        "eventlog_evtx_report_failed_logons",
        "Report Windows failed logon events (4625) from an EVTX file (restricted to allowed roots).",
        ToolSchema.Object(
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="EventLogEvtxFailedLogonsReportTool"/> class.
    /// </summary>
    public EventLogEvtxFailedLogonsReportTool(EventLogToolOptions options) : base(options) { }
    ...
    protected override Task<string> InvokeCoreAsync(JsonObject? arguments, CancellationToken cancellationToken) {
        ...
        if (!SecurityEvtxQueryExecutor.TryBuildFailedLogons(request, out var queryResult, out var failure, cancellationToken)) {
        ...
        var response = EventLogEvtxSecurityReportHelper.BuildTopUserResponse(
            arguments: arguments,
            model: queryResult,
            title: "Failed logons: top target users (preview)",
