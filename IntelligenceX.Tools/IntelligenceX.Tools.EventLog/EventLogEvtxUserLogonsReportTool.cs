...
/// <summary>
/// Builds a summary report for Windows logon events from an EVTX file (read-only, restricted to allowed roots).
/// </summary>
public sealed class EventLogEvtxUserLogonsReportTool : EventLogToolBase, ITool {
    private static readonly ToolDefinition DefinitionValue = new(
        "eventlog_evtx_report_user_logons",
        "Report Windows user logon-related events (4624/4625/4634/4647) from an EVTX file (restricted to allowed roots).",
        ToolSchema.Object(
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="EventLogEvtxUserLogonsReportTool"/> class.
    /// </summary>
    public EventLogEvtxUserLogonsReportTool(EventLogToolOptions options) : base(options) { }
    ...
    protected override Task<string> InvokeCoreAsync(JsonObject? arguments, CancellationToken cancellationToken) {
        ...
        if (!SecurityEvtxQueryExecutor.TryBuildUserLogons(request, out var queryResult, out var failure, cancellationToken)) {
        ...
        var response = EventLogEvtxSecurityReportHelper.BuildTopUserResponse(
            arguments: arguments,
            model: queryResult,
            title: "User logons: top target users (preview)",
