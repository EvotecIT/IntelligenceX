# Runs a repeatable batch of chat scenarios using the real host runtime.

[CmdletBinding()] param(
    [string] $ScenarioDir = '.\IntelligenceX.Chat\scenarios',
    [string] $Filter = 'ad-*-10-turn.json',
    [string[]] $Tags,
    [string] $OutDir = '.\artifacts\chat-scenarios',
    [string[]] $AllowRoot,
    [switch] $NoBuild,
...
Write-Header 'Run Chat Scenario Suite'
...
Write-Step ("Continue on error (per scenario): {0}" -f $ContinueOnError)
...
for ($i = 0; $i -lt $scenarioRuns.Count; $i++) {
    ...
    Write-Header ("Scenario {0}/{1}: {2}" -f ($i + 1), $scenarioRuns.Count, $scenario.Name)
    ...
    try {
        ...
        Write-Fail ("Scenario failed: {0}" -f $scenario.Name)
        Write-Fail $_.Exception.Message
    }

    if ($scenarioSucceeded) {
        $passed++
        Write-Step ("Scenario passed: {0}" -f $scenario.Name)
    } else {
        $failed++
...
Write-Header 'Scenario Suite Summary'
