...
- `Build-Project.ps1`
  ...
  - `-PackagesOnly` uses the focused ChartForgeX-style `project.build.json` lane and builds both `IntelligenceX` and its required `IntelligenceX.Shared` dependency
...
- `project.build.json`
  - focused public NuGet package build used by `Build-Project.ps1 -PackagesOnly`
  - owns matching `0.1.x` versions and publication order for `IntelligenceX.Shared` and `IntelligenceX`
...
The checked-in `release.json` packages `IntelligenceX.Shared` before the main `IntelligenceX` package and uses the unified `UploadReady` staging + Winget manifest flow for desktop-app release handoff.
...
When you run `Build-Project.ps1 -PackagesOnly` without overriding `-ConfigPath`, the wrapper invokes the focused `Build\project.build.json` package lane directly. This keeps public NuGet packaging independent from private workspace validation, desktop-app Winget assets, and installer tooling. Pass an explicit release config only when you intentionally need the wider release graph.
