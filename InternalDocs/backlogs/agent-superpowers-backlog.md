...
- [x] (P1) Extend `eventlog_live_query` and `eventlog_evtx_query` with first-class structured filters (`keywords`, `level`, `user_id`, `event_record_id`, named-data include/exclude) so AD investigations do not rely on XPath-only filtering.
