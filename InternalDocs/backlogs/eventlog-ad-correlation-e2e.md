...
- [x] Batch 1: add contract tests for `eventlog_named_events_query` `meta.entity_handoff` shape, caps, and required metadata.
...
- [x] Pick helper tool name: `ad_handoff_prepare` or `ad_identity_candidates_prepare`.
- [x] Pick helper location: `IntelligenceX.Tools.ADPlayground` or shared utility namespace.
- [x] Decide helper input contract: require `meta.entity_handoff` only or allow raw EventLog rows fallback.
