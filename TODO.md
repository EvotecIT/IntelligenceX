...
- [ ] PR #985: Reviewer todo/critical keeps reporting missing legacy `enable_*_pack` migration despite explicit migration mapping in `SqliteServiceProfileStore` plus targeted regressions for `enable_testimox_pack`, `enable_officeimo_pack`, and `enable_powershell_pack`; review context is truncated (`30/55 files`, `2 patches trimmed to 20000 chars`), so treat as churn unless maintainers escalate with a failing repro. Source: https://github.com/EvotecIT/IntelligenceX/pull/985#issuecomment-3984087852
