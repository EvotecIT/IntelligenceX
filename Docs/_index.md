---
title: IntelligenceX Documentation
description: Explore IntelligenceX docs for reviewer onboarding, IX Chat, tool packs, CLI, .NET and PowerShell integrations, Project Ops, security, and troubleshooting.
slug: index
---

# IntelligenceX Documentation

IntelligenceX is a zero-trust AI platform for GitHub review automation, desktop chat, local tool packs, and developer integrations.
Use the sections below to jump straight to the capability you need.

## Start Here

Begin with the fastest orientation path into the platform:

- [Getting Started](/docs/getting-started/)
- [Security & Trust](/docs/security/)
- [FAQ](/docs/faq/)
- [Troubleshooting](/docs/troubleshooting/)
- [Reviewer Onboarding Wizard](/docs/reviewer/onboarding-wizard/)
- [CLI Quick Start](/docs/cli/quickstart/)

## GitHub Actions Reviewer

Set up structured AI reviews for pull requests while keeping identity, secrets, and workflow ownership in your own GitHub environment.

- [Reviewer Overview](/docs/reviewer/overview/)
- [Understanding Reviewer Output](/docs/reviewer/review-output/)
- [Reviewer Onboarding Wizard](/docs/reviewer/onboarding-wizard/)
- [Reviewer Web Setup UI](/docs/reviewer/setup-web/)
- [Reviewer Configuration](/docs/reviewer/configuration/)
- [Reviewer Security & Trust](/docs/reviewer/security-trust/)
- [Reviewer Static Analysis](/docs/reviewer/static-analysis/)
- [Workflow vs JSON](/docs/reviewer/workflow-vs-json/)
- [Reviewer Projects + PR Monitoring](/docs/reviewer/projects-pr-monitoring/)
- [Reviewer Project Bootstrap and Sync](/docs/reviewer/project-bootstrap-sync/)
- [Reviewer Project Views and Operations](/docs/reviewer/project-views-and-ops/)

## IX Chat

Run IntelligenceX locally as a Windows tray app with selectable runtimes, persistent threads, and tool calling.

- [IX Chat Overview](/docs/chat/overview/)
- [IX Chat Quickstart](/docs/chat/quickstart/)
- [IX Chat Architecture](/docs/chat/architecture/)
- [Windows Tray Chat App](/docs/apps/windows-tray-chat/)
- [Chat: Local Providers](/docs/apps/chat-local-providers/)

## Tool Packs and Developer Interfaces

Build on top of IntelligenceX with the CLI, the .NET library, the PowerShell module, and reusable tool packs.

### Tool Packs

- [IX Tools Overview](/docs/tools/overview/)
- [Tool Catalog](/docs/tools/catalog/)
- [Tool Governance](/docs/tools/governance/)

### CLI Tools

- [CLI Overview](/docs/cli/overview/)
- [CLI Quick Start](/docs/cli/quickstart/)
- [CLI Install](/docs/cli/install/)
- [Usage Reporting](/docs/cli/usage-reporting/)

### .NET Library

- [Library Overview](/docs/library/overview/)
- [Library Quickstart](/docs/library/quickstart/)
- [Library Providers](/docs/library/providers/)
- [Library Tool Calling](/docs/library/tools/)
- [Library Tool Packs](/docs/library/tool-packs/)
- [API Reference](/api/)

### PowerShell Module

- [PowerShell Overview](/docs/powershell/overview/)
- [PowerShell Quickstart](/docs/powershell/quickstart/)
- [PowerShell Cookbook](/docs/powershell/cookbook/)
- [PowerShell Troubleshooting](/docs/powershell/troubleshooting/)
- [Cmdlet Reference](/api/powershell/)

## Project Ops

Project Ops gives maintainers first-party GitHub issue and project workflows for triage, backlog cleanup, and project field sync.

- [Project Ops Overview](/docs/project-ops/overview/)
- [Project Ops Demo Capture Runbook](/docs/project-ops/demo-capture-runbook/)
- [Project Ops Related Docs](/docs/project-ops/related-docs/)
- [Projects + PR Monitoring](/docs/reviewer/projects-pr-monitoring/)
- [Project Views and Operations](/docs/reviewer/project-views-and-ops/)

## Examples and Visual References

Use these pages when you want working samples, screenshots, or common fixes.

- [Examples](/docs/examples/)
- [Screenshots](/docs/screenshots/)
- [Troubleshooting](/docs/troubleshooting/)
- [FAQ](/docs/faq/)
