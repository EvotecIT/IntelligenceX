# Plugin Rollout (Open + Private Packs)
...
A plugin should be loadable from a folder (primary runtime model), optionally distributed as NuGet for developers.

Runtime expectation:
- plugin folder contains entry assembly, dependency assemblies, and manifest
- host loads from folder path under `plugins/`

Distribution options:
- app bundle with pre-expanded plugin folders (recommended for end users)
- NuGet package (optional) that can be expanded into plugin folder form
...
- `IntelligenceX.Tools.PowerShell` (engine-backed via `IntelligenceX.Engines.PowerShell`)
...
2. Move private packs (`System`, `TestimoX`, `AD`) to TestimoX repo and publish from there.
