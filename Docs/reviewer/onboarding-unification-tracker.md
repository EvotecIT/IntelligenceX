...
| Setup test maintainability split (autodetect/post-apply groups) | Completed | IX Tests | Split large `Program.Setup.cs` into focused partial files (`Program.Setup.AutodetectContract.cs`, `Program.Setup.PostApply.cs`) to reduce monolith size and keep static-analysis maintainability checks actionable. |
| Post-apply verification constants for check names/statuses | Completed | IX CLI + IX Tests | Added shared `SetupPostApplyCheckNames`/`SetupPostApplyCheckValues` constants and switched post-apply tests to use them; also replaced parse-based fixed timestamp with deterministic constructor-based `DateTimeOffset`. |
| Post-apply verifier maintainability split | Completed | IX CLI | Split `SetupPostApplyVerify.cs` models/constants into `SetupPostApplyVerify.Models.cs` so verifier logic stays under maintainability thresholds while preserving behavior. |
